...
struct InputNode : nos::NodeContext
{
	nosUUID ChannelPinId{};
	nos::bluefish::TChannelInfo ChannelInfo{};
	
	InputNode(const nosFbNode* node) : NodeContext(node)
	{
	...
	void OnMenuCommand(nosUUID itemID, uint32_t cmd) override
	{
		...
		if (BERR_NO_ERROR != err)
		{
			nosEngine.LogE("Unable to open input channel: %s", bfcUtilsGetStringForBErr(err));
			return;
		}
		...
		channelPin.video_mode = command.VideoMode;
	...
	void OpenChannel()
	{
		...
		nosEngine.LogI("Route input %s", channelStr.c_str(), modeStr.c_str());
